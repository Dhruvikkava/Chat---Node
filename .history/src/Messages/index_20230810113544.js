  ...
  socket.on('joinRoom', async(roomData) => {   
    try {
        ...
        if(checkUser.length != 0){
            ...
            const newCreatedData = new chatRoomMembers({{room_id: checkUserJoined[0]._id, user_id:roomData.userId}})
            newCreatedData.save((err, book) => {
              if (err) {
                console.error('Error:', err);
              } else {
                // Populate the author reference
                Book.populate(book, { path: 'author', model: 'Author' }, (err, populatedBook) => {
                  if (err) {
                    console.error('Error:', err);
                  } else {
            ...
            let createdData = await chatRoomMembers.create({room_id: checkUserJoined[0]._id, user_id:roomData.userId}).populate('room_id')
            socket.join(createdData.unique_room_id);
            
            // let roomId = new mongoose.Types.ObjectId(createdData._id);
            console.log("ddddf2333",createdData)
            // let checkUserAleradyJoined = await chatRoomMembers.findOne({user_id:roomData.userId, room_id:roomId})
            let findSocketUser = await users.find((userDetails) => userDetails.user._id == roomData.userId);
            io.to(findSocketUser.socket_id).emit('created_room_id', {createdRoomId: createdData.unique_room_id});
        }
