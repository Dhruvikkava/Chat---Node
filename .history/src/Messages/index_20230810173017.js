...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', async(userData) => {   
    try {
        ...
        roomMembers[createRoom.unique_room_id] = userData._id;
        let roomDetails = await chatRoomMembers.aggregate([     
          { $match: { user_id: userId } },
          {
          $lookup: {
  ...
  socket.on('joinRoom', async(roomData) => {   
    try {
        ...
        if(checkUser.length != 0){
            ...
            roomMembers[checkUserJoined[0].unique_room_id] = roomData.userId;
            ...
            roomMembers[createdNewData.room_id.unique_room_id] = roomData.userId;
