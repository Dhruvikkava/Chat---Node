...
io.on('connection', (socket) => {
  ...
  socket.on('joinRoom', async(roomData) => {   
    try {
        let checkUserJoined = await chatRoomMembers.findOne({room_id: roomData.roomId, user_id:roomData.userId})
        if(checkUserJoined){
            console.log("ddddf",checkUserJoined)
