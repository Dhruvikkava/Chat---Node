...
io.on('connection', (socket) => {
  ...
  socket.on('message', async(message) => {   
    try {
        ...
        socket.to(message.roomId).broadcast.emit('get_all_room_data',{response: roomDetails});  
    } catch (error) {
  ...
  socket.on('joinRoom', async(roomData) => {   
    try {
        ...
        if(checkUser.length != 0){
            ...
            socket.to(checkUserJoined[0].unique_room_id).broadcast.emit('get_all_room_data',{response: roomDetails});
        }else{
            ...
            socket.to(createdNewData.room_id.unique_room_id).broadcast.emit('get_all_room_data',{response: roomDetails});            
        }
        let roomId = new mongoose.Types.ObjectId(checkUserJoined[0]._id);
