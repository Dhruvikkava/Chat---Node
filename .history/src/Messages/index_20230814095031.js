...
io.on('connection', (socket) => {
  ...
  socket.on('message', async(message) => {   
    try {
        ...
        //     console.log("inner exist room data",element.room_id, element.user_id)
        //     let receiverData = await chatReceiver.findOne({roomId: new mongoose.Types.ObjectId(element.room_id), receiverId: new mongoose.Types.ObjectId(element.user_id), isRead: false})
        //     console.log("ddddd",receiverData)
        //     if(receiverData){
        //       console.log("gggggggggggggg")
        //       io.to(userData.socket_id).emit('unreadUserMessage', receiverData);
        //     }
        //   }          
        // });
        let receiverData = await chatReceiver.findOne({roomId: new mongoose.Types.ObjectId(roomData._id), isRead: false})
        console.log("ttttttttt", receiverData)
        socket.broadcast.to(message.roomId).emit('unreadUserMessage', receiverData);
