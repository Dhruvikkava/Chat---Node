...
io.on('connection', (socket) => {
  ...
  socket.on('message', async(message) => {   
    try {
        ...
        let addUser = await chatMessagesModels.create({userId: userDetails['user']._id, message:message})
        console.log("ssss",addUser)
        const getUserData = await chatMessagesModels.findById(userDetails.user._id);
        console.log("first",getUserData)
