...
io.on('connection', (socket) => {
  ...
  socket.on('joinRoom', async(roomData) => {   
    try {
        ...
        let userRoomId = new mongoose.Types.ObjectId(checkUserJoined[0]._id);
        let checkUser = checkUserJoined[0].roomMembersData.filter(((el) => {return el.user_id == roomData.userId && el.room_id == roomData.roomId}))
