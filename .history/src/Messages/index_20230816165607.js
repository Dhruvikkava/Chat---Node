...
io.on('connection', (socket) => {
        ...
        roomMembersData.forEach(async(element) => {
          ...
          if(element.user_id != message.userId){
            let userData = users.find((data) => data.user._id == element.user_id)
              ...
              console.log("nnnnnnnn23",element)
              let isExistUserRoom = roomMembers.find((el) => {return el.room_id == roomData.unique_room_id && el.user_id == element.user_id})
  ...
  socket.on('createRoom', async(userData) => {   
    try {
        ...
        let createRoom = await chatRooms.create({unique_room_id: generateUniqueString, roomName: userData.roomName})
  ...
  socket.on('loadMoreItems', async(roomData) => {
    let checkUserJoined = await chatRooms.aggregate([
      { $match: { unique_room_id: roomData.roomId } },
      {
          $lookup: {
