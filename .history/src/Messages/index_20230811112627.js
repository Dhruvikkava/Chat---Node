  ...
  socket.on('joinRoom', async(roomData) => {   
    try {
        ...
        if(checkUser.length != 0){
            ...
            io.to(checkUserJoined[0].unique_room_id).broadcast.emit('get_all_room_data',{response: roomDetails});
        }else{
            ...
            io.to(createdNewData.room_id.unique_room_id).broadcast.emit('get_all_room_data',{response: roomDetails});            
        }
        let roomId = new mongoose.Types.ObjectId(checkUserJoined[0]._id);
