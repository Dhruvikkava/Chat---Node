...
io.on('connection', (socket) => {
  ...
  socket.on('message', async(message) => {   
    try {
        let roomId = await chatRooms.findOne({unique_room_id: message.roomId})
        let addUser = await chatMessagesModels.create({userId: message.userId, message:message.message, roomId:})
  ...
  socket.on('createRoom', async(userData) => {   
    try {
        ...
        if(createdUserData){
          ...
          io.to(findSocketUser.socket_id).emit('created_room_id', {roomId: createRoom.unique_room_id});
        }
  ...
  socket.on('joinRoom', async(roomData) => {   
    try {
        ...
        if(checkUserJoined.length != 0){
            ...
            io.to(findSocketUser.socket_id).emit('created_room_id', {roomId: checkUserJoined[0].unique_room_id});
        }else{
            let createdData = await chatRoomMembers.create({room_id: roomData.roomId, user_id:roomData.userId})
            socket.join(createdData.unique_room_id);
            console.log("ddddf2333",createdData.unique_room_id)
            let findSocketUser = await users.find((userDetails) => userDetails.user._id == roomData.userId);
            io.to(findSocketUser.socket_id).emit('created_room_id', {roomId: createdData.unique_room_id});
        }
