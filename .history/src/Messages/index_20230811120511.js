  ...
  socket.on('message', async(message) => {   
    try {
        ...
        if(roomMembersData.length == existRoomMembers.length){
          let roomId = new mongoose.Types.ObjectId(roomData._id)
        ...
        let userId = new mongoose.Types.ObjectId(message.userId);
        let roomDetails = await chatRoomMembers.aggregate([     
          { $match: { user_id: userId } },
          {
            $lookup: {
        ...
        io.to(message.roomId).emit('get_all_room_data',{response: roomDetails});  
    } catch (error) {
  ...
  socket.on('joinRoom', async(roomData) => {   
    try {
        ...
        if(checkUser.length != 0){
            ...
            io.to(checkUserJoined[0].unique_room_id).emit('get_all_room_data',{response: roomDetails});
            ...
            io.to(createdNewData.room_id.unique_room_id).emit('get_all_room_data',{response: roomDetails});     
